...

{
  ...
  assert.match(profileHtml, /id="agent-approval-tier"[\s\S]*?value="1"[\s\S]*?value="2"[\s\S]*?value="3"/, '必须提供默认审批三档选择且 1 档在前');
...

{
  ...
  assert.match(profileJs, /agent_approval_tier:\s*agentApprovalTier\.value/, '保存完整配置时必须提交默认审批档位');
