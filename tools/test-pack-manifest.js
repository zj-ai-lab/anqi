...
//
// 两条判据并存、都要拦截，而不是二选一：
// ① "from 目录下是否存在 package.json"——这是主判据，不随本机磁盘状态漂移：
//   package.json 是随仓库检出（clone/checkout）就存在的文件（不是"git 追踪"这个
//   动作本身在做判断——这里就是普通 fs.existsSync 查工作树，只是工作树里这份
//   文件的存在不依赖有没有跑过 npm install，这一点与 node_modules 不同）。
// ② "from 目录下是否存在 node_modules"——这曾经是这里唯一的判据，单独用时在
//   一次全新 clone / CI checkout（从未跑过 npm ci --ignore-scripts 装 runtime
//   依赖）上这棵目录天然不存在，会让断言在唯一真正会把坑带进发行产物的环境里
//   必然放行、只在开发者本机凑巧手动装过依赖时才生效——但反过来单独留着①也有
//   盲区：万一某个 from 目录下有 node_modules 却没有 package.json（例如把
//   node_modules 手工搬到别处、package.json 留在别的层级），①会看不见。两条都
//   查，任意一条命中就拦截，本机状态漂移只影响②是否命中，不影响①，合起来才是
//   真正不随环境漂移的守卫。
for (const entry of extraResources) {
  if (!entry || typeof entry.from !== 'string') continue;
  const fromDir = path.join(REPO_ROOT, entry.from);
  const packageJsonAtFrom = path.join(fromDir, 'package.json');
  const nodeModulesAtFrom = path.join(fromDir, 'node_modules');
  assert.ok(
  ...
  assert.ok(
    !fs.existsSync(nodeModulesAtFrom),
    `build.extraResources 条目 from:"${entry.from}" 的目录下已经存在字面名为 "node_modules" 的子目录——即使这里没有 package.json，electron-builder 的 extraResources 复制同样会把它整棵静默跳过（判据同上一条断言，见本文件顶部注释）。请把 "from" 指向更高一级目录。`
  );
}
