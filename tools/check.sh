...
# 与临时库，跑完自行收尾。
echo "[39/45] 前端行为冒烟（安全 Markdown + counts.agent 特性探测门 + agent_* 五键往返 + SSE 帧到 DOM 映射静态审查）"
