...
**它是什么：** 打开某个案件，页面上的「AI 助理」抽屉就是一个绑定本案的 agent 会话——这个案件的案件夹就是它的工作区，它只看得到、动得了这一个案件；无论对话里怎么说，都切换不到别的案件。引擎是内置的 DSH（进程外 sidecar），供应商不写死，DeepSeek 官方或任何 OpenAI 兼容端点都可以。
